...
class TaskDetailItem extends StatefulWidget {
  ...
  final ValueChanged<double>? onChecked;
...
class _TaskDetailItemState extends State<TaskDetailItem>
  ...
  ///Этот таймер нужен потому что hero анимация длится примерно 1 секунду, после завершения анимации выполняется анимация появления списка
  ///Однако если страница деталей задачи открыта из страницы "Завершенные задачи", то hero анимации нет, и этот timer не нужен
  Timer? timer;
  ...
  void dispose() {
    timer?.cancel();
    _controller.dispose();
    debugPrint("taskDetailItem ${widget.index} уничтожен");
    super.dispose();
  }
                  ...
                  child: Checkbox(
                    ...
                    onChanged: (value) {
                      ...
                      if (widget.onChecked != null) {
                        widget.onChecked?.call(currentProgress);
                      }
                    },
