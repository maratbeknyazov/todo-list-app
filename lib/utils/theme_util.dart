...
class ThemeUtil {
  ...
  static ThemeUtil getInstance() => _instance ??= ThemeUtil._internal();
  ...
  ThemeData getTheme(
    ...
    if (themeBean == null) {
      throw ArgumentError('themeBean cannot be null');
    }
  ...
  ThemeData _getThemeData(Color? color, String? themeType) {
    if (color == null || themeType == null) {
      throw ArgumentError('color and themeType cannot be null');
    }
  ...
  Color getDarkColor(Color color) {
    int number = 20;
    final int baseRed = (color.r * 255.0).round() & 0xff;
    final int baseGreen = (color.g * 255.0).round() & 0xff;
    final int baseBlue = (color.b * 255.0).round() & 0xff;
    int red = (baseRed - number).clamp(0, 255).toInt();
    int green = (baseGreen - number).clamp(0, 255).toInt();
    int blue = (baseBlue - number).clamp(0, 255).toInt();
