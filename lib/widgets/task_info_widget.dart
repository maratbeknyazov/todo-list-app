...
class TaskInfoWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final iconColor = isCardChangeWithBg
        ...
        : ColorBean.fromBean(taskIconBean?.colorBean ?? ColorBean());
    final textColor = getTextColor(context);
    final taskIconData = IconBean.fromBean(taskIconBean?.iconBean ?? IconBean());
  ...
  Widget getStatusWidget(BuildContext context, Color taskColor) {
    ...
    if (startDate.isNotEmpty && deadLine.isNotEmpty) {
      ...
      ///Если текущее время меньше установленного времени начала
      if (now.isBefore(begin)) {
        return getBeginIcon(begin, now, context, taskColor);
      }

      ///Если текущее время находится в пределах установленного периода
  ...
  Color getTextColor(BuildContext context){
    ...
    return DefaultTextStyle.of(context).style.color ?? Colors.black;
  }
